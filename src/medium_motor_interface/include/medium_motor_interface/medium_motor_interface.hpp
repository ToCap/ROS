...
/**
 * @class MediumMotorInterface
 * @brief Hardware interface for a single LEGO Mindstorms EV3 Medium Motor.
 *
 * Implements all five native EV3 Medium Motor block modes, mapped to
 ...
 *
 * Hardware specifications (EV3 Medium Motor):
 *   - Encoder resolution : 1 degree per tick
 *   - No-load speed      : ~240–260 RPM  (~25.1–27.2 rad/s)
 *   - Stall torque       : ~8 N·cm
 *   - Power range        : -100 to 100 (%)
 *   - Positive power     : clockwise rotation
 *   - Negative power     : counter-clockwise rotation
 *
 * @note The Medium Motor is faster but less powerful than the Large Motor.
 *       It is typically used for lighter mechanisms (arms, sensors mounts).
 *
 ...
 *   - "max_rpm"     double   No-load speed in RPM             [default: 250.0]
 *   - "invert"      bool     Invert motor direction           [default: false]
 */
class MediumMotorInterface : public hardware_interface::SystemInterface
{
public:
  RCLCPP_SHARED_PTR_DEFINITIONS(MediumMotorInterface)
