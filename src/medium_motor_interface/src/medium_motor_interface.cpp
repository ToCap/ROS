#include "medium_motor_interface/medium_motor_interface.hpp"
#include <cmath>
#include <stdexcept>
#include <pluginlib/class_list_macros.hpp>

namespace medium_motor_interface
{
...
/// Default no-load speed for an EV3 Medium Motor [RPM].
static constexpr double kDefaultMaxRpm = 250.0;
...
CallbackReturn MediumMotorInterface::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  {
    RCLCPP_ERROR(
      rclcpp::get_logger("MediumMotorInterface"),
  ...
  if (info.hardware_parameters.count("max_rpm"))
  {
    ...
    if (max_rpm <= 0.0)
    {
      RCLCPP_ERROR(rclcpp::get_logger("MediumMotorInterface"),
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("MediumMotorInterface"),
    "MediumMotorInterface initialized — joint: '%s', max_velocity: %.2f rad/s (%.1f RPM), "
    "direction: %s",
...
MediumMotorInterface::export_state_interfaces()
{
...
MediumMotorInterface::export_command_interfaces()
{
...
return_type MediumMotorInterface::read(
  const rclcpp::Time & /*time*/,
  const rclcpp::Duration & period)
{
  /*
   *  +-----------------------+        +-----------------------+
   *  |   ROS 2 Controller    |        |  MediumMotorInterface  |
   *  |                       |<-------| read()                |
   *  |                       | pos    |  position [rad]       |
   *  |                       | vel    |  velocity [rad/s]     |
   *  |                       | eff    |  effort   [%]         |
...
// ─────────────────────────────────────────────────────────────────────────────
// write  — EV3 Medium Motor block logic
// ─────────────────────────────────────────────────────────────────────────────

return_type MediumMotorInterface::write(
  const rclcpp::Time & /*time*/,
  const rclcpp::Duration & /*period*/)
{
  /*
  ...
  if (std::abs(degrees_cmd_) > kDegreeDeadBand)
  {
    ...
    if (!target_reached_)
    {
      ...
      else
      {
        // Target reached
        stop_motor();
        RCLCPP_DEBUG(rclcpp::get_logger("MediumMotorInterface"),
...
PLUGINLIB_EXPORT_CLASS(
  medium_motor_interface::MediumMotorInterface,
