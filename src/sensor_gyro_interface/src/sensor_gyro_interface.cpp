...
hardware_interface::CallbackReturn SensorGyroInterface::on_init(
...
std::vector<hardware_interface::StateInterface> SensorGyroInterface::export_state_interfaces()
{
  ...
  state_interfaces.emplace_back(
      hardware_interface::StateInterface("gyro", "angle", &gyro_angle_));
  state_interfaces.emplace_back(
      hardware_interface::StateInterface("gyro", "rate", &gyro_rate_));
  return state_interfaces;
}

std::vector<hardware_interface::CommandInterface> SensorGyroInterface::export_command_interfaces()
{
  // Pas de commande, uniquement lecture du capteur
  return {};
}
    ...
    const rclcpp::Time & /*time*/, const rclcpp::Duration &period)
{
  ...
  gyro_rate_ = 45.0 * std::sin(simulate_angle_ / 180.0 * M_PI);  // [°/s]
...
hardware_interface::return_type SensorGyroInterface::write(
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
