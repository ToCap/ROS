...
/**
 * @class SensorUltrasonicInterface
 * @brief Hardware interface implementation for a Lego Mindstorm ultrasonic sensor (EV2 or EV3)
 *
 * This class provides a ROS 2 hardware interface for controlling and monitoring
 * a Lego Mindstorm ultrasonic sensor(EV2 or EV3). It supports initialization,
 * state and command interface exports, and data exchange between ROS 2 Control
 * and the physical hardware - only for simulation.
 *
 */
class SensorUltrasonicInterface : public hardware_interface::SystemInterface
{

public:
  RCLCPP_SHARED_PTR_DEFINITIONS(SensorUltrasonicInterface)


  ~SensorUltrasonicInterface();
   ...
   *
   * The EV3 ultrasonic sensor is considered as passive and does not accept commands,
